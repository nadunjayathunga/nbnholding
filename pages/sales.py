...
from data import time_series_data, fin_tiles_values, company_info, months
    ...
    children=[
        dbc.Row(
            children=[
                ...
                dbc.Col(
                    children=[
                        html.H5('SALES PERSON ACHIEVEMENT',
                                className='text-center text-primary mb-4'),
...
@callback(
    [Output(component_id='cust-select', component_property='options'),
     ...
     Output(component_id='rev-internal-external-ytd',component_property='figure'),
     ...
     Output(component_id='modal-total-customers',component_property='is_open'),
     Output(component_id='modal-new-customers', component_property='is_open'),
     Output(component_id='modal-inactive-customers',component_property='is_open'),
    ...
    [
        ...
        Input(component_id='dJobs', component_property='data'),
        Input(component_id='fGL', component_property='data'),
        Input(component_id='dEmployee', component_property='data'),
        Input(component_id='dCustomers', component_property='data'),
        Input(component_id='fGlJob', component_property='data'),
        Input(component_id='exp_allocation', component_property='data'),
        ...
        Input(component_id='btn-total-customers-open', component_property='n_clicks'),
        Input(component_id='btn-total-customers-close', component_property='n_clicks'),
        Input(component_id='btn-new-customers-open', component_property='n_clicks'),
        Input(component_id='btn-new-customers-close', component_property='n_clicks'),
        Input(component_id='btn-inactive-customers-open', component_property='n_clicks'),
        Input(component_id='btn-inactive-customers-close', component_property='n_clicks'),
    ...
    [State(component_id='modal-total-customers', component_property='is_open'),
     ...
     State(component_id='modal-inactive-customers',component_property='is_open')
     ],
    prevent_initial_call=True
)
def data_output(dCoAAdler,dJobs, fGL, dEmployee, dCustomers, fGlJob, exp_allocation, database,end_date, cust_select, time_freq, b1, b2, is_open_1, b3, b4, is_open_2, b5, b6, is_open_3):
    df_dcoa_adler = pd.DataFrame.from_dict(dCoAAdler)
    df_djobs = pd.DataFrame.from_dict(dJobs)
    ...
    df_dCustomers = pd.DataFrame.from_dict(dCustomers)
    ...
    df_merged = pd.merge(
        left=df_merged[['bussiness_unit_name', 'cost_center', 'voucher_date', 'voucher_number', 'credit', 'debit',
                        'ledger_name', 'forth_level', 'third_level', 'second_level', 'first_level', 'job_number',
                        'net', 'period', 'transaction_type', 'ledger_code']],
    ...
    df_merged = pd.merge(left=df_merged, right=df_dEmployee[
        ['emp_id', 'emp_name', 'leave_policy', 'travel_cost', 'ba', 'hra', 'tra', 'ma', 'oa', 'pda']],
                         on='emp_id', how='left')
    ...
    cust_ledger: list = list(
        df_dCustomers.loc[df_dCustomers[
                              'customer_code'] == cust_select, 'ledger_code'])  # list all the ledger account each customer_code has
    ...
    last_col_date_filt = (df_fGl['transaction_type'] == 'Receipt') & (
        df_fGl['ledger_code'].isin(
            cust_ledger))  # create a filter which show receipts from a given customer for whole period
    col_filt = (df_fGl['transaction_type'] == 'Receipt') & (
        df_fGl['ledger_code'].isin(list(df_dCustomers['ledger_code']))) & (
                       df_fGl['voucher_date'] >= cy_begin_date) & (df_fGl[
                                                                       'voucher_date'] <= cy_end_date)  # for YTD period this list all the receips from customers
    ...
    if database in ['nadunjayathunga$elite_security', 'nadunjayathunga$premium']:
        ...
        df_fGlJobs = pd.DataFrame.from_dict(fGlJob)
        df_fGlJobs['voucher_date'] = pd.to_datetime(df_fGlJobs['voucher_date'])
        df_exp_allo = pd.DataFrame.from_dict(exp_allocation)
        df_exp_allo['date'] = pd.to_datetime(df_exp_allo['date'])
    ...
    df_merged['customer_type'] = df_merged.groupby(
        'customer_code')['voucher_date'].transform(
        'min')  # to get the minimum voucher_date for each customer_code group
    ...
    bar_chart_rev_new_existing = px.bar(data_frame=result,
                                        ...
                                        color='customer_type',
                                        barmode='stack',
                                        labels={'month_year': 'Period', 'net': 'QAR Value',
    ...
    sales_src_df = pd.merge(left=sales_src_df[['net', 'period', 'ledger_code']], right=df_dcoa_adler[[
        'ledger_code', 'second_level']], on='ledger_code',
                            how='left')  # to get the customer ledger code for a given customer code
    ...
    pie_chart_rev_ytd_source.update_traces(
        labels=[{'Due from Related Parties': 'In-House', 'Trade Receivables': 'Market'}[label] for label in
                pie_chart_rev_ytd_source.data[0]['labels']])
    ...
    pie_chart_col_ytd.update_traces(
        labels=[{'Due from Related Parties': 'In-House', 'Trade Receivables': 'Market'}[label] for label in
                pie_chart_col_ytd.data[0]['labels']])

    df_fGlJobs['job_number'] = df_fGlJobs.apply(order_id, axis=1)
    df_fGlJobs['net'] = df_fGlJobs['credit'] - df_fGlJobs['debit']
    df_GlJob_merged = pd.merge(left=df_fGlJobs[['ledger_code', 'voucher_date', 'net', 'job_number']],
                               right=df_dcoa_adler[[
...
def budget_area(end_date, rev_type, dCoAAdler, fGL, fBudget):
    df_dcoa_adler = pd.DataFrame.from_dict(dCoAAdler)
    df_fGl = pd.DataFrame.from_dict(fGL)
    df_fGl['voucher_date'] = pd.to_datetime(df_fGl['voucher_date'])
